...
import { createNotebookArtifact, publishNotebook } from "./handlers.ts";
...
it.effect("creates and publishes a notebook artifact using only invocation-owned scope", () => {
  ...
  const store = NotebookRevisionStore.of({
    ...
    save: (input) => Effect.succeed({ ...revision, documentId: input.documentId }),
  ...
  return Effect.gen(function* () {
    const result = yield* createNotebookArtifact({
    ...
    expect(result).toMatchObject({
      ...
      messageId: expect.stringMatching(/^artifact-message-/),
      documentId: expect.stringMatching(/^notebook-/),
