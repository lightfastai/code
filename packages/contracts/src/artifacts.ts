...
const ARTIFACT_TITLE_MAX_CHARS = 255;
const SCENE_OBJECTS_MAX = 128;

export const ChatArtifactId = TrimmedNonEmptyString.check(
  Schema.isMaxLength(ARTIFACT_ID_MAX_CHARS),
  Schema.isPattern(/^[a-z0-9_-]+$/i),
);
export type ChatArtifactId = typeof ChatArtifactId.Type;

const SceneObjectId = TrimmedNonEmptyString.check(
...
const SceneColor = TrimmedNonEmptyString.check(
...
const SceneLabel = TrimmedNonEmptyString.check(Schema.isMaxLength(120));
...
export const ArtifactSourceReference = Schema.Struct({
  ...
  locator: Schema.optional(TrimmedNonEmptyString.check(Schema.isMaxLength(1024))),
});
...
export const ChatScene3DArtifact = Schema.Struct({
  ...
  id: ChatArtifactId,
  kind: Schema.Literal("3d-scene"),
  schemaVersion: Schema.Literal(1),
  title: TrimmedNonEmptyString.check(Schema.isMaxLength(ARTIFACT_TITLE_MAX_CHARS)),
  payload: Scene3DPayload,
  provenance: Schema.optional(Schema.Array(ArtifactSourceReference).check(Schema.isMaxLength(32))),
...
export const ChatArtifactAttachment = Schema.Union([ChatScene3DArtifact]);
